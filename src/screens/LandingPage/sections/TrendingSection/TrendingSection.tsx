...
export const TrendingSection = (): JSX.Element => {
  ...
  const handleWishlistToggle = (e: React.MouseEvent, product: { id: number; image: string; title: string; price: string }) => {
    e.preventDefault();
    e.stopPropagation();
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6 w-full max-w-[1440px] mx-auto">
        {products.map((product) => (
          <div key={product.id} className="relative">
            ...
            >
              <Heart className={`w-4 h-4 ${isInWishlist(product.id.toString()) ? "fill-white" : ""}`} />
            </button>
            <Card
